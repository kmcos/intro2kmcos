...
integer(kind=iint), parameter, public :: O = 0
...
integer(kind=iint), parameter, public :: O2_ads_right = 1
integer(kind=iint), parameter, public :: O2_ads_up = 2
integer(kind=iint), parameter, public :: O2_des_right = 3
integer(kind=iint), parameter, public :: O2_des_up = 4
...
subroutine put_O_fcc100_hol(site)
    ...
    if(avail_sites(O2_ads_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_ads_right, site)
    endif

    if(avail_sites(O2_ads_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_ads_right, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(O2_ads_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_ads_up, site)
    endif

    if(avail_sites(O2_ads_up, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_ads_up, site + (/0, -1, 0, 0/))
    endif

    ! enable affected processes
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(O)
        call add_proc(O2_des_right, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(O)
        call add_proc(O2_des_right, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(O)
        call add_proc(O2_des_up, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(O)
        call add_proc(O2_des_up, site + (/0, -1, 0, 0/))
    end select
...
subroutine take_O_fcc100_hol(site)
    ...
    ! disable affected processes
    if(avail_sites(O2_des_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_des_right, site)
    endif

    if(avail_sites(O2_des_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_des_right, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(O2_des_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_des_up, site)
    endif

    if(avail_sites(O2_des_up, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_des_up, site + (/0, -1, 0, 0/))
    endif

    ! enable affected processes
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(empty)
        call add_proc(O2_ads_right, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(O2_ads_right, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(empty)
        call add_proc(O2_ads_up, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(O2_ads_up, site + (/0, -1, 0, 0/))
    end select
...
subroutine touchup_fcc100_hol(site)
    ...
    if (can_do(O2_ads_right, site)) then
        call del_proc(O2_ads_right, site)
    endif
    if (can_do(O2_ads_up, site)) then
        call del_proc(O2_ads_up, site)
    endif
    if (can_do(O2_des_right, site)) then
        call del_proc(O2_des_right, site)
    endif
    if (can_do(O2_des_up, site)) then
        call del_proc(O2_des_up, site)
    endif
