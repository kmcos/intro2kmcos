...
integer(kind=iint), parameter, public :: CO_oxi_00 = 3
integer(kind=iint), parameter, public :: CO_oxi_01 = 4
integer(kind=iint), parameter, public :: CO_oxi_02 = 5
integer(kind=iint), parameter, public :: CO_oxi_03 = 6
integer(kind=iint), parameter, public :: O2_ads_00 = 7
integer(kind=iint), parameter, public :: O2_ads_01 = 8
integer(kind=iint), parameter, public :: O2_des_00 = 9
integer(kind=iint), parameter, public :: O2_des_01 = 10
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'O2_adsdes' was written by                  |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (fcc100)
                    call replace_species((/i, j, k, fcc100_hol/), null_species, default_species)
                end select
            end do
        end do
    end do
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                select case(layer)
                case(fcc100)
                    call touchup_fcc100_hol((/i, j, k, fcc100_hol/))
                end select
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(CO_ads)
        call put_CO_fcc100_hol(lsite)

    case(CO_des)
        call take_CO_fcc100_hol(lsite)

    case(CO_oxi_00)
        call take_CO_fcc100_hol(lsite)
        call take_O_fcc100_hol(lsite + (/1, 0, 0, 0/))

    case(CO_oxi_01)
        call take_CO_fcc100_hol(lsite)
        call take_O_fcc100_hol(lsite + (/0, 1, 0, 0/))

    case(CO_oxi_02)
        call take_CO_fcc100_hol(lsite + (/1, 0, 0, 0/))
        call take_O_fcc100_hol(lsite)

    case(CO_oxi_03)
        call take_CO_fcc100_hol(lsite + (/0, 1, 0, 0/))
        call take_O_fcc100_hol(lsite)

    case(O2_ads_00)
        call put_O_fcc100_hol(lsite)
        call put_O_fcc100_hol(lsite + (/1, 0, 0, 0/))

    case(O2_ads_01)
        call put_O_fcc100_hol(lsite)
        call put_O_fcc100_hol(lsite + (/0, 1, 0, 0/))
...
subroutine put_CO_fcc100_hol(site)
    ...
    if(avail_sites(CO_ads, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_ads, site)
    endif

    if(avail_sites(O2_ads_00, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_ads_00, site)
    endif

    if(avail_sites(O2_ads_00, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_ads_00, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(O2_ads_01, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_ads_01, site)
    endif

    if(avail_sites(O2_ads_01, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_ads_01, site + (/0, -1, 0, 0/))
    endif

    ! enable affected processes
    call add_proc(CO_des, site)
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(O)
        call add_proc(CO_oxi_00, site)
    end select
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(O)
        call add_proc(CO_oxi_02, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(O)
        call add_proc(CO_oxi_03, site + (/0, -1, 0, 0/))
    end select


end subroutine put_CO_fcc100_hol

subroutine take_CO_fcc100_hol(site)
    ...
    ! disable affected processes
    if(avail_sites(CO_des, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_des, site)
    endif

    if(avail_sites(CO_oxi_00, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxi_00, site)
    endif

    if(avail_sites(CO_oxi_01, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxi_01, site)
    endif

    if(avail_sites(CO_oxi_02, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxi_02, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(CO_oxi_03, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxi_03, site + (/0, -1, 0, 0/))
    endif
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(O2_ads_00, site + (/-1, 0, 0, 0/))
    end select
    ...
    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(O2_ads_01, site + (/0, -1, 0, 0/))
    end select


end subroutine take_CO_fcc100_hol

subroutine put_O_fcc100_hol(site)
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(CO)
        call add_proc(CO_oxi_00, site + (/-1, 0, 0, 0/))
    case(O)
        call add_proc(O2_des_00, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(CO)
        call add_proc(CO_oxi_01, site + (/0, -1, 0, 0/))
    case(O)
        call add_proc(O2_des_01, site + (/0, -1, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(CO)
        call add_proc(CO_oxi_02, site)
    case(O)
        call add_proc(O2_des_00, site)
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(CO)
        call add_proc(CO_oxi_03, site)
    case(O)
        call add_proc(O2_des_01, site)
    end select


end subroutine put_O_fcc100_hol

subroutine take_O_fcc100_hol(site)
    ...
    ! disable affected processes
    if(avail_sites(CO_oxi_00, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxi_00, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(CO_oxi_01, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxi_01, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(CO_oxi_02, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxi_02, site)
    endif

    if(avail_sites(CO_oxi_03, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxi_03, site)
    endif

    if(avail_sites(O2_des_00, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_des_00, site)
    endif

    if(avail_sites(O2_des_00, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_des_00, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(O2_des_01, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_des_01, site)
    endif

    if(avail_sites(O2_des_01, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_des_01, site + (/0, -1, 0, 0/))
    endif
...
end subroutine take_O_fcc100_hol

subroutine touchup_fcc100_hol(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(CO_ads, site)) then
        call del_proc(CO_ads, site)
    endif
    if (can_do(CO_des, site)) then
        call del_proc(CO_des, site)
    endif
    if (can_do(CO_oxi_00, site)) then
        call del_proc(CO_oxi_00, site)
    endif
    if (can_do(CO_oxi_01, site)) then
        call del_proc(CO_oxi_01, site)
    endif
    if (can_do(CO_oxi_02, site)) then
        call del_proc(CO_oxi_02, site)
    endif
    if (can_do(CO_oxi_03, site)) then
        call del_proc(CO_oxi_03, site)
    endif
    if (can_do(O2_ads_00, site)) then
        call del_proc(O2_ads_00, site)
    endif
    if (can_do(O2_ads_01, site)) then
        call del_proc(O2_ads_01, site)
    endif
    if (can_do(O2_des_00, site)) then
        call del_proc(O2_des_00, site)
    endif
    if (can_do(O2_des_01, site)) then
        call del_proc(O2_des_01, site)
    endif
...
end subroutine touchup_fcc100_hol
